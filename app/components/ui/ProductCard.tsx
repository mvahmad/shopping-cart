...
const stars = (value = 0) =>
    Array.from({ length: 5 }).map((_, i) => (
        <svg
            key={i}
            viewBox="0 0 24 24"
            className={`h-4 w-4 ${i < Math.round(value) ? "fill-yellow-400" : "fill-gray-300"}`}
        >
            <path d="M12 .587l3.668 7.431 8.2 1.192-5.934 5.786 1.4 8.164L12 18.896l-7.334 3.864 1.4-8.164L.132 9.21l8.2-1.192L12 .587z" />
        </svg>
    ));

export default function ProductCard({ p, showRating = true }: { p: Product; showRating?: boolean }) {
            ...
            <div className="relative rounded-lg border border-blue-100 bg-white/95 shadow-sm shadow-blue-100/60 hover:shadow-md hover:shadow-blue-200 transition-shadow">
                <div className="absolute inset-x-0 top-2 flex items-center justify-between px-3">
                    ...
                    <button
                        ...
                        className="grid h-9 w-9 place-items-center rounded-full bg-white/90 text-blue-500 shadow border border-blue-100 hover:bg-white transition active:scale-95"
                    >
                        <svg viewBox="0 0 24 24" className="h-5 w-5">
                            <path
                                d="M12.001 4.529c2.349-2.532 6.297-2.532 8.646 0 2.35 2.533 2.35 6.64 0 9.172l-7.071 7.626a2 2 0 0 1-2.95 0L3.555 13.7c-2.35-2.533-2.35-6.64 0-9.173 2.348-2.532 6.296-2.532 8.646 0z"
                                className={(fav ? "fill-rose-500" : "fill-transparent stroke-2 stroke-rose-400") + " transition-colors duration-200"}
                            />
                        </svg>
                    </button>
                </div>
                    ...
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center">
                            <div className="flex flex-col items-end justify-center ">
                                {p.oldPrice && (
                                    <div className="text-xs text-slate-400 line-through">{toIRR(p.oldPrice)}</div>
                                )}
